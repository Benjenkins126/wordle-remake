...
function isWordInList(currentRow) {
    let finalWord = "";
    for(let x = 0; x < 6; x++) {
        let currentCharacter = $('.wordleEntry[data-row="' + currentRow + '"][data-column="' + (x + 1) + '"]').val();
        finalWord += currentCharacter;
    }

    if(words.includes(finalWord.toLowerCase())) {
        return true;
    } else {
...
//Step through code and make sure all edge cases are fixed